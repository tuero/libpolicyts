// File: concepts.h
// Description: Utility helper for custom concepts and typetraits

#ifndef LIBPTS_CONCEPTS_H_
#define LIBPTS_CONCEPTS_H_
 ...
 */
template <class T>
typename std::add_rvalue_reference_t<T> makeval();
...
using static_switch = typename std::tuple_element_t<N, std::tuple<T...>>;
...
/**
 * Concept to check if a type as a solution probability
 */
template <typename T>
concept HasSolutionProb = requires(T t) {
