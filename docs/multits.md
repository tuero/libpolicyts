...
MultiTS is a rollout sampling algorithm, which samples actions for rollouts of a fixed depth, 
...
MultiTS needs as input a valid environment and model with policy evaluations.
...
template <typename T>
concept IsMultiTSModel = IsLubyModelValue<T> || IsLubyModelVector<T>;
```

## Example

Full example in `examples/multits/`.
...
multits::SearchInput<State, Policy<4>> search_input{
...
multits::SearchInput<State, Policy> search_input{
    .puzzle_name = "puzzle_0",
    .state = start_state,
    .search_budget = budget,
    .depth = depth,
